...
export default function AdminLogin() {
  ...
  useEffect(() => {
    if(localStorage.getItem('beauty_admin_token')) router.push('/admin/dashboard')
  }, [])
  ...
  return (
    <>
      <Head><title>Admin Login — Beauty Store</title></Head>
      <div style={{fontFamily:'DM Sans, sans-serif',background:'linear-gradient(135deg,#1A0A0F 0%,#2C0A1A 100%)',minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
        <div style={{background:'#fff',borderRadius:'28px',padding:'48px 44px',width:'100%',maxWidth:'420px',boxShadow:'0 30px 80px rgba(0,0,0,0.3)'}}>
          <div style={{fontFamily:'Cormorant Garamond, serif',fontSize:'30px',fontWeight:600,color:'var(--rose)',textAlign:'center',marginBottom:'6px'}}>Beauty <em style={{fontStyle:'italic',color:'var(--gold)'}}>Beauty</em></div>
          ...
          <div style={{marginBottom:'18px'}}>
            ...
            <input type="email" placeholder="admin@beautystore.com" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&login()} style={{width:'100%',padding:'14px 16px',border:'1.5px solid var(--border)',borderRadius:'12px',fontFamily:'DM Sans, sans-serif',fontSize:'14px',color:'var(--text)',outline:'none',background:'#FAFAFA'}}/>
          </div>
